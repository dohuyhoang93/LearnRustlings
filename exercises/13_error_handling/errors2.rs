...
fn total_cost(item_quantity: &str) -> Result<i32, ParseIntError> {
    ...
    //Cách 1: sử dụng toán tử `?` để truyền lỗi lên trên (propagate lỗi lên caller) - idiomatic Rust
    let qty = item_quantity.parse::<i32>()?;

    //Cách 2: Sử dụng `match` cơ bản
    // let qty = item_quantity.parse::<i32>();
    //     match qty {
    ...
    //hoặc không cần `match` vì  ::<i32>() đã trả về 1 Result(T,E) rồi:
